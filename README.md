...
| 日期 | 版本 | 更新内容 |
|------|------|----------|
| 2026-03-10 | `2026.3.10-ws` | <ul><li>新增企业微信智能机器人**长连接（WebSocket）模式**，无需公网 IP，适合内网/NAS 部署</li><li>新增环境变量 `WECOM_STREAM_MODE`、`WECOM_BOT_ID`、`WECOM_BOT_SECRET` 等，与 Webhook 模式互斥切换</li><li>更新 `docker-compose.yml` 透传长连接相关变量</li><li>更新 README 企业微信配置文档，增加双模式对比说明</li></ul> |
...
| 对比项 | Webhook 模式（传统）| 长连接模式（WebSocket）|
|--------|-------------------|----------------------|
| 连接方式 | HTTP 回调 | WebSocket 长连接 |
| 公网需求 | **需要**公网可访问 URL | **无需**公网 IP |
| 消息加解密 | 需要 | 无需 |
| 延迟 | 较高 | 低，实时性好 |
| 凭证类型 | Token + EncodingAESKey | BotID + Secret |
| 推荐场景 | 有公网环境 | 内网/NAS 部署 |

---

### 模式一：Webhook 模式（需要公网 URL）

#### 获取凭证
1. 访问 [企业微信管理后台](https://work.weixin.qq.com/)
2. 进入「应用管理」，用 API 模式创建「智能机器人」应用
3. 在「接收消息」配置中获取 Token 和 EncodingAESKey
4. 配置回调 URL（需公网可访问）
...
在 [`openclaw.json`](README.md:648) 的 [`channels.wecom`](openclaw.json.example:154) 下使用字典结构，每个 key 为账号 ID（如 `bot1`、`bot2`）：
...
或通过环境变量 `WECOM_BOTS_JSON`（单行 JSON）：
...
### 模式二：长连接模式（WebSocket，无需公网 IP）

> 适合 NAS、内网服务器等无公网 IP 的场景。由机器人主动建立到企业微信服务器的 WebSocket 长连接，无需配置回调 URL。

#### 获取凭证
1. 访问 [企业微信管理后台](https://work.weixin.qq.com/)
2. 进入「应用管理」→「智能机器人」，创建机器人
3. 在机器人配置页选择「API 模式」→「**长连接**」
4. 获取 **BotID** 和 **Secret**（注意：与 Webhook 的 Token/EncodingAESKey **不同**，不可混用）
...
# WECOM_HEARTBEAT_INTERVAL=30000   # 心跳间隔 ms，默认 30 秒
# WECOM_RECONNECT_RETRIES=5        # 断线重连次数，默认 5 次
```

#### 注意事项

- `WECOM_STREAM_MODE=true` 时，`WECOM_TOKEN` / `WECOM_ENCODING_AES_KEY` 会被忽略
- `WECOM_STREAM_MODE=false`（默认）时，走 Webhook 模式，`WECOM_BOT_ID` / `WECOM_BOT_SECRET` 会被忽略
- 两种模式互斥，不能同时生效
...
docker run -d \
  ...
  # 或改用长连接模式（二选一）：
  # -e WECOM_STREAM_MODE=true \
  # -e WECOM_BOT_ID=your-bot-id \
  # -e WECOM_BOT_SECRET=your-bot-secret \
